...
(function() {
  ...
  API.LinkedPi = LinkedPi = (function(superClass) {
    extend(LinkedPi, superClass);

    function LinkedPi() {
      this.rollback = bind(this.rollback, this);
      this.backup = bind(this.backup, this);
      this.defaults = bind(this.defaults, this);
      return LinkedPi.__super__.constructor.apply(this, arguments);
    }

    LinkedPi.prototype.idAttribute = "pi_id";

    LinkedPi.prototype.fields = 'host port enabled'.split(' ');

    LinkedPi.prototype.defaults = function() {
      return {
        host: remote_host,
        port: remote_port,
    ...
    LinkedPi.prototype.backup = function() {
      return this.backup_attributes = this.toJSON();
    };

    LinkedPi.prototype.rollback = function() {
      if (this.backup_attributes) {
        this.set(this.backup_attributes);
        return this.backup_attributes = void 0;
      }
  ...
  API.View.EditLinkedMaster = EditLinkedMaster = (function(superClass) {
    extend(EditLinkedMaster, superClass);
    ...
    function EditLinkedMaster() {
      ...
      this.$fv = bind(this.$fv, this);
      this.$f = bind(this.$f, this);
      return EditLinkedMaster.__super__.constructor.apply(this, arguments);
    }

    EditLinkedMaster.prototype.$f = function(field) {
      return this.$("[name='" + field + "']");
    };

    EditLinkedMaster.prototype.$fv = function() {
      var field, ref, val;
      field = arguments[0], val = 2 <= arguments.length ? slice.call(arguments, 1) : [];
      return (ref = this.$f(field)).val.apply(ref, val);
    };

    EditLinkedMaster.prototype.initialize = function(options) {
      this.edit = options.edit;
      ($('body')).append(this.$el.html(get_template('link-modal')));
      (this.$('.modal-header .close')).remove();
      (this.$el.children(":first")).modal();
      this.model.backup();
      this.model.bind('change', this.render);
    ...
    EditLinkedMaster.prototype.render = function() {
      var field, k, len, ref;
      this.undelegateEvents();
    ...
    EditLinkedMaster.prototype.viewmodel = function() {
    ...
    EditLinkedMaster.prototype.events = {
      'submit form': 'save',
      'click .cancel': 'cancel',
      'change': 'change',
      'keyup': 'change'
    ...
    EditLinkedMaster.prototype.save = function(e) {
      var save;
      e.preventDefault();
      this.viewmodel();
    ...
    EditLinkedMaster.prototype.validate = function(e) {
      var errors, field, fn, k, len, ref, results, that, v, validators;
      that = this;
      validators = {
        host: (function(_this) {
        ...
        port: (function(_this) {
          return function(v) {
            if (v <= 0 || v > 65536) {
              return 'please enter a valid port number (1..65536)';
            }
          };
        })(this)
      };
      ...
      for (k = 0, len = errors.length; k < len; k++) {
        ref = errors[k], field = ref[0], v = ref[1];
        (this.$('[type=submit]')).prop('disabled', true);
        (this.$(".control-group.host")).addClass('warning');
        results.push((this.$(".control-group.host .controls")).append($("<span class='help-inline warning'>" + v + "</span>")));
      }
      return results;
    };

    EditLinkedMaster.prototype.cancel = function(e) {
      this.model.rollback();
      if (!this.edit) {
        this.model.destroy();
      }
